...
router.post('/', requireRole('strategist'), async (req, res) => {
  const { name, link, tiktok_link, priority_weight = 3, all_vibes = false, vibe_ids = [] } = req.body;
...
router.patch('/:id', requireRole('strategist'), async (req, res) => {
  const { name, link, tiktok_link, status, priority_weight, all_vibes, vibe_ids } = req.body;
  let rows;
  try {
    ({ rows } = await db.query(`
      UPDATE songs SET
